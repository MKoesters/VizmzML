...
def match_df_to_mz_i_list(
    ...
    for i, row in fragment_df.iterrows():
        if rel_cutoff:
            cutoff = row['mz'] * rel_cutoff
        else:
            cutoff = dalton_cutoff
        res = (row['mz'], 'max_intensity', row['name'] + ' ' + ';'.join(row['mods']))
        if (abs(mz - row['mz']) < cutoff).any():
            matches.append(res)
        else:
...
def update_figure(spec_id, seq, df_data, df_row):
    ...
    if df_row is not None and len(df_row) > 0:
        ...
        # fac.new_plot(MS_precision=20e-6)
        for index in df_row:
            table_seq = df_data[index]['Sequence']
            table_mods = df_data[index]['Modifications']
            if table_mods is not None:
                table_seq_mods = '#'.join([table_seq, table_mods])
            else:
                table_seq_mods = table_seq
            fragger = peptide_fragmentor.PeptideFragment0r(table_seq_mods, ions=['y', 'b'])
            df = fragger.df
            if len(df) > 0:
                df = df[df['series'].isin(['y', 'b'])][['seq', 'series', 'mz', 'name', 'mods']]
                matches, mismatches = match_df_to_mz_i_list(df, spec_peaks, rel_cutoff=20e-6)
                if len(matches) > 0:
                    fac.add(
                        matches,
                        style='label.triangle.MS_precision',
                        name='fragment matches {0}'.format(table_seq_mods),
                        color=(0, 255, 0)
                    )
                # if len(mismatches) > 0:
                #     fac.add(
                #         mismatches,
                #         style='label.triangle.MS_precision',
                #         name='fragment mismatches {0}'.format(table_seq_mods),
                #         color=(255, 0, 0)
                #     )
                # breakpoint()
                # data_tmp = fac.save()
